...
- Standardize each invidiual tag
...
combined_data <- messy_data %>% 
  left_join(museum_attributes, by = "museum")
```
...
```{r, echo = TRUE}
temp_table <- clean_data_obj %>% 
  left_join(clean_creation) %>% 
  left_join(clean_artists)
```
...
Here, I've expanded "circa" to mean around 5 years before or after. "after 1636" could have a number of different meantings depending on the context - maybe we can limit it based on the last year of the studio's activity. 

## Gotchas

- If you're dealing with times, not just dates... then watch out for timezones. Python and R both have specialized libraries for these.
...
- Mixing uncertainty across different fields
  - i.e. having a checkmark to say a record is "done" isn't very informative
...
Data modeling can get infinitely complicated if you want to accomodate every possible use case. If you're not a museum or library, don't do that.
...
clean_data_obj %>% 
  separate_rows(tags, sep=";") %>% 
  left_join(museum_attributes, by = "museum") %>% 
  ggplot(aes(ymin = year_early, ymax = year_late, x = tags, color = city, group = acq_no)) +
  geom_errorbar(position="dodge", width = 0.2)
```

We need to decide how to handle year of creation. Some visualizations let us incorporate timespan info, but we could also take the midpoint between the boundaries, depending on our question.

## Network of artists who worked together and when

Joining a table to itself can give us combinations of artists who worked together on the same artwork. This could be used to create a network dataset to analyze.

```{r, echo = TRUE}
clean_creation %>% 
  left_join(clean_creation, by = "acq_no") %>% 
  filter(name.x != name.y) %>% 
  left_join(clean_data_obj, by = "acq_no") %>% 
...
Working in groups, draft a data scheme for encoding this auction catalogue. Aside from the obvious, think about:
